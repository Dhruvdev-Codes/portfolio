  ...
  return (
    <>
      ...
      <div className="fixed bottom-3 left-3 right-3 sm:left-auto sm:right-6 sm:bottom-6 z-50 animate-in fade-in slide-in-from-bottom-4 duration-200">
        <div className="w-full sm:w-[460px] h-[78vh] sm:h-[540px] max-h-[620px] bg-[#12161f]/98 backdrop-blur-xl border border-[#222e44] rounded-xl flex flex-col shadow-2xl overflow-hidden font-mono">
          {/* Terminal Header */}
          <div className="flex items-center justify-between px-4 py-3 bg-[#161d2b] border-b border-[#222e44]">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => setMessages([])}
                className="p-1.5 rounded text-slate-400 hover:text-slate-100 hover:bg-[#1f2a3e] cursor-pointer transition-colors"
                title="Reset session"
              >
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3 text-xs">
                  ...
                  <div className="space-y-1.5">
                    {promptSuggestions.map((prompt, idx) => (
                      <button
                        ...
                        className="w-full text-left p-2.5 rounded-lg bg-[#161d2b] hover:bg-[#1d2738] text-slate-300 hover:text-cyan-300 border border-[#222e44] text-[11px] cursor-pointer transition-colors"
                      >
                        ▸ {prompt}
                      </button>
                    ))}
            ...
            {messages.map((m) => (
              <div
                ...
                className={`p-3 rounded-lg ${
                  m.role === "user"
                    ? "bg-[#182131] text-slate-100 ml-4 sm:ml-6 border border-cyan-500/40"
                    : "bg-[#141a26] border border-[#222e44] text-slate-200 mr-2 sm:mr-4"
                }`}
              >
            ...
            {isLoading && (
              <div className="flex items-center gap-2 text-cyan-400 animate-pulse text-xs py-1 font-mono">
          ...
          <form onSubmit={handleSubmit} className="p-3 bg-[#161d2b] border-t border-[#222e44] flex gap-2">
            <input
              ...
              className="flex-1 bg-[#111722] border border-[#222e44] rounded-lg px-3 py-2.5 text-xs text-slate-200 focus:outline-none focus:border-cyan-400 font-sans"
            />
            <button
              type="submit"
              disabled={isLoading || !input.trim()}
              className="bg-gradient-to-r from-cyan-500 to-teal-500 hover:from-cyan-400 hover:to-teal-400 disabled:opacity-50 text-slate-950 font-bold px-3.5 py-2.5 rounded-lg text-xs flex items-center justify-center transition-colors cursor-pointer"
            >
