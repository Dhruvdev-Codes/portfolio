...
export function Navbar({ onOpenTerminal }: NavbarProps) {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-40 transition-all duration-200 ${
        scrolled || mobileMenuOpen
          ? "bg-[#12161f]/95 backdrop-blur-md border-b border-[#1e2738] shadow-lg shadow-black/50"
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-2.5">
          ...
          <button
            ...
            className="md:hidden p-2 rounded-lg bg-[#182030] border border-[#222e44] hover:border-cyan-500/50 text-slate-300 hover:text-cyan-400 transition-colors"
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#12161f]/98 border-b border-[#222e44] px-4 py-5 font-mono shadow-2xl backdrop-blur-xl">
          <div className="flex flex-col space-y-2 mb-4">
            {navLinks.map((link) => (
              <a
                ...
                className="px-3 py-2.5 rounded-lg bg-[#161d2b] border border-[#222e44] text-xs font-semibold text-slate-200 hover:text-cyan-300 hover:border-cyan-500/40 transition-colors flex items-center justify-between"
              >
          ...
          <div className="flex items-center justify-between pt-3 border-t border-[#1e2738] text-xs text-slate-400">
            <div className="flex items-center gap-2">
              <Activity className="w-3.5 h-3.5 text-teal-400 animate-pulse" />
              <span>Latency: <span className="text-cyan-400 font-semibold">{latency}ms</span></span>
            </div>
            <button
              onClick={() => {
              ...
              className="flex items-center gap-1.5 text-cyan-400 hover:text-cyan-300 text-xs font-semibold cursor-pointer"
            >
